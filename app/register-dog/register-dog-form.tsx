...
export function RegisterDogForm() {
  ...
  return <form action={action} className="grid gap-5">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <label className="font-black text-navy">Kennel Club name <span className="text-sm text-charcoal/50">optional</span><input name="kennelClubName" className="mt-2 w-full rounded-2xl border border-cocoa/10 px-4 py-3" /></label>
      ...
      <BreedSelector />
      <label className="font-black text-navy">DNA confirmed<select name="dnaConfirmed" defaultValue="UNKNOWN" className="mt-2 w-full rounded-2xl border border-cocoa/10 px-4 py-3"><option value="UNKNOWN">Not provided</option><option value="YES">Yes</option><option value="NO">No</option></select></label>
    ...
    <fieldset className="rounded-[1.5rem] border border-cocoa/10 bg-white/70 p-4"><legend className="px-2 font-black text-navy">Dog Type</legend><p className="text-sm font-bold text-charcoal/55">Select every type that applies.</p><div className="mt-3 flex flex-wrap gap-3">{dogTypes.map((type) => <label key={type} className="rounded-full bg-lightgrey px-4 py-2 text-sm font-black text-cocoa"><input type="checkbox" name="dogTypes" value={type} className="mr-2" />{type}</label>)}</div></fieldset>
    <Submit />
  </form>;
}
