...
export function BarkBoothLogo({ iconOnly = false }: { iconOnly?: boolean }) {
  return <span className="inline-flex items-center gap-3" aria-label="Bark Booth official registry mark"><span className="relative grid h-12 w-12 place-items-center rounded-2xl border border-navy/20 bg-offwhite shadow-registry"><span className="absolute left-2 right-2 top-2 h-px bg-navy/20" /><span className="absolute left-2 top-4 h-1.5 w-5 rounded-full bg-sand/70" /><span className="absolute bottom-2 right-2 h-1.5 w-3 rounded-full bg-navy/15" /><span className="text-xl leading-none" aria-hidden="true">🐾</span></span>{!iconOnly && <span><span className="block font-heading text-xl font-bold tracking-tight text-navy">Bark Booth</span><span className="registry-label block text-[0.58rem]">Canine Registry</span></span>}</span>;
}
