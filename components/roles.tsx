...
export function DirectoryListingCard({ listing }: { listing: { publicDisplayName: string | null; organisationName: string | null; professionalCategory: ProfessionalCategory | null; description: string | null; serviceArea: string | null; website: string | null; email: string | null; showEmailPublicly: boolean; requestedRole: RequestedRole; status: RoleApplicationStatus; isFeatured: boolean; featuredUntil: Date | null; promotionTier: PromotionTier } }) {
  ...
  return <Card className={activePromotion ? "border border-rosette/25 bg-gradient-to-br from-white to-biscuit/40" : undefined}><div className="flex flex-wrap gap-2"><VerificationBadge role="PROFESSIONAL" />{activePromotion && <RoleBadge label="Featured Professional" />}</div><h3 className="mt-3 text-2xl font-bold text-navy">{listing.publicDisplayName || listing.organisationName}</h3>{listing.professionalCategory && <p className="mt-1 font-bold text-rosette">{listing.professionalCategory.toLowerCase().replace(/_/g, " ")}</p>}<p className="mt-3 leading-7 text-charcoal/65">{listing.description}</p><div className="mt-4 grid gap-2 text-sm font-bold text-charcoal/65">{listing.serviceArea && <span>Service area: {listing.serviceArea}</span>}{website && <a className="text-rosette" href={website}>Website</a>}{listing.showEmailPublicly && listing.email && <a className="text-rosette" href={`mailto:${listing.email}`}>Contact</a>}</div></Card>;
}
