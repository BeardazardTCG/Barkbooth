  ...
  return <>
    <Section eyebrow="Canine Identity Record" title={dog.name}>
      <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <Card><h2 className="text-2xl font-bold text-navy">Full profile</h2><p className="mt-3 leading-7 text-charcoal/65">Each editable section saves only when you press its labelled button.</p><div className="mt-5 rounded-2xl bg-lightgrey p-4"><p className="text-sm font-bold text-navy">Profile photo</p>{canManage && <><ManagedForm action={uploadDogProfilePhoto} encType="multipart/form-data" resetOnSuccess className="mt-3 grid min-w-0 gap-3"><input type="hidden" name="dogId" value={dog.id} /><input type="hidden" name="replacing" value={String(Boolean(dog.profilePhoto))} /><FileField name="photo" label="Choose profile photo" required accept="image/jpeg,image/png,image/webp" maxBytes={5 * 1024 * 1024} /><FormSubmitButton label={dog.profilePhoto ? "Replace photo" : "Upload photo"} pendingLabel="Uploading…" /></ManagedForm>{dog.profilePhoto && <ManagedForm action={removeDogProfilePhoto} warnOnLeave={false} className="mt-3 grid gap-2"><input type="hidden" name="dogId" value={dog.id} /><FormSubmitButton label="Remove photo" pendingLabel="Removing…" requireDirty={false} className="bg-white text-red-700" /></ManagedForm>}</>}</div><div className="mt-5 flex flex-wrap gap-2"><a href="#pet-details" className="rounded-full bg-navy px-4 py-2 text-sm font-bold text-white">Pet details</a><a href="#records" className="rounded-full bg-white px-4 py-2 text-sm font-bold text-navy">Records</a><a href="#behaviour" className="rounded-full bg-white px-4 py-2 text-sm font-bold text-navy">Behaviour</a><ButtonLink href="/dogs">Back to My Dogs</ButtonLink></div></Card>
      </div>
    </Section>

    {canManage && <Section eyebrow="Pet profile" title="Edit pet details"><Card><ManagedForm action={updatePetDetails} className="grid gap-4" ><input type="hidden" name="dogId" value={dog.id} /><div id="pet-details" className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 font-bold text-navy">Dog name<input name="name" required defaultValue={dog.name} className="rounded-2xl p-3" /></label><label className="grid gap-2 font-bold text-navy">Kennel Club name<input name="kennelClubName" defaultValue={dog.kennelClubName ?? ""} className="rounded-2xl p-3" /></label><label className="grid gap-2 font-bold text-navy">Breed<input name="breed" defaultValue={dog.breed ?? ""} className="rounded-2xl p-3" /></label><label className="grid gap-2 font-bold text-navy">Date of birth<input type="date" name="dateOfBirth" defaultValue={dog.dateOfBirth?.toISOString().slice(0, 10) ?? ""} className="rounded-2xl p-3" /></label><label className="grid gap-2 font-bold text-navy">Sex<input name="sex" defaultValue={dog.sex ?? ""} className="rounded-2xl p-3" /></label><label className="grid gap-2 font-bold text-navy">Colour<input name="colour" defaultValue={dog.colour ?? ""} className="rounded-2xl p-3" /></label><label className="grid gap-2 font-bold text-navy">Country of registration<input name="countryOfRegistration" defaultValue={dog.countryOfRegistration ?? ""} className="rounded-2xl p-3" /></label><label className="grid gap-2 font-bold text-navy">Visibility<select name="visibility" defaultValue={dog.visibility} className="rounded-2xl p-3"><option value="PUBLIC">Public registry identity</option><option value="PRIVATE">Private to your account</option><option value="LINK_ONLY">Link only</option></select></label></div><FormSubmitButton label="Save pet details" /></ManagedForm></Card></Section>}
