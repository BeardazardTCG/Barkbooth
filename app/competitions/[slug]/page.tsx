...
export default async function CompetitionPage({ params }: { params: { slug: string } }) {
  ...
  return <>
    ...
    {competition.judges.length > 0 && <Section eyebrow="Competition panel" title="Meet the judges"><div className="grid gap-4 md:grid-cols-2">{competition.judges.map(judge=><Card key={judge.id}><p className="registry-label">{judge.guestJudge?"Guest judge":judge.roleTitle||"Judge"}</p><h2 className="mt-2 text-2xl font-bold text-navy">{judge.name}</h2>{judge.organisation&&<p className="font-bold text-info">{judge.organisation}</p>}{judge.biography&&<p className="mt-3">{judge.biography}</p>}{judge.profession&&<p className="mt-3"><strong>Professional work:</strong> {judge.profession}</p>}{judge.judgingFocus&&<p className="mt-3"><strong>Judging focus:</strong> {judge.judgingFocus}</p>}</Card>)}</div></Section>}
