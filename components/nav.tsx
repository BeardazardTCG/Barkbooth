...
export function Nav() {
  return <header className="sticky top-0 z-20 border-b border-navy/10 bg-cream/90 backdrop-blur"><nav className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3"><Link href="/" className="flex items-center gap-2"><BarkBoothLogo /></Link><div className="hidden items-center gap-1 overflow-x-auto md:flex">{links.slice(1).map(([label, href]) => <Link key={href} href={href} className="whitespace-nowrap rounded-full px-3 py-2 text-sm font-bold text-navy/75 hover:bg-lightgrey hover:text-navy">{label}</Link>)}</div><Link href="/upload" className="rounded-full bg-pink px-5 py-3 text-sm font-black text-white shadow-soft">Enter</Link></nav></header>;
}

export function SiteFooter() {
  return <footer className="border-t border-navy/10 bg-white/70 px-5 py-8"><div className="mx-auto max-w-6xl"><p className="font-black text-navy">Bark Booth trust placeholders</p><p className="mt-2 text-sm text-charcoal/65">Skill-based judged competitions only. No raffles. No random prize draws. Animal welfare is paramount.</p><div className="mt-4 flex flex-wrap gap-2">{footerLinks.map(([label, href]) => <Link key={href} href={href} className="rounded-full bg-lightgrey px-3 py-2 text-xs font-black text-navy">{label}</Link>)}</div></div></footer>;
}

export function MobileTabs() {
  return <div className="fixed inset-x-3 bottom-3 z-30 grid grid-cols-5 rounded-[1.5rem] bg-navy/95 p-2 text-center text-[11px] font-bold text-white shadow-soft md:hidden"><Link href="/upload" className="rounded-2xl bg-pink px-1 py-2">Enter</Link>{[["Comps", "/competitions"], ["Choice", "/peoples-choice-pooch"], ["Calendar", "/calendar-pups"], ["Winners", "/winners"]].map(([label, href]) => <Link key={href} href={href} className="rounded-2xl px-1 py-2 hover:bg-white/10">{label}</Link>)}</div>;
}
