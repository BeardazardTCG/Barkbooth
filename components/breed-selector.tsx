...
type BreedSelectorProps = { mixedBreed?: boolean; initialBreed?: string | null; initialBreedMix?: string | null; compact?: boolean };

export function BreedSelector({ mixedBreed = false, initialBreed = "", initialBreedMix = "", compact = false }: BreedSelectorProps) {
  ...
  return <div className={`${compact ? "grid gap-3 sm:col-span-2" : "grid gap-4 rounded-[1.5rem] border border-navy/10 bg-white/70 p-4 md:col-span-2"}`}>
    ...
    <div>
      ...
      {!compact && <p className="mt-2 text-sm font-bold text-charcoal/55">Search the full alphabetised breed list, or choose Mixed Breed or Other.</p>}
    </div>
    ...
    {isMixed && <fieldset className="min-w-0">
      ...
      <div className="mt-3 max-h-48 overflow-y-auto rounded-2xl border border-navy/10 bg-white p-2 sm:max-h-64">
