...
import { HeroUploadForm, PendingForm } from "@/components/competition-action-form";
import { requireUser } from "@/lib/auth/session";
import { moderateEntry, publishResults, saveResult, transitionCompetition, saveJudge, removeJudge, savePrize, removePrize, uploadCompetitionHero, removeCompetitionHero, uploadJudgeImage, removeJudgeImage } from "@/lib/competitions/actions";
...
export default async function ManageCompetitionPage({ params }: { params: { id: string } }) {
  ...
  const competition = await prisma.competition.findUnique({
    where: { id: params.id },
    include: { prizes: { orderBy: { displayOrder: "asc" } }, judges: { orderBy: { displayOrder: "asc" } }, entries: { include: { dog: true }, orderBy: { submittedAt: "asc" } } },
  });
  if (!competition) notFound();

  return <>
    <Section eyebrow="Restricted administration" title={`Manage ${competition.title}`}>
      <Card className="mb-5"><p className="registry-label">Current state</p><h2 className="text-2xl font-bold text-navy">{competition.status}</h2><div className="mt-4 grid gap-3 sm:flex sm:flex-wrap">{({DRAFT:[["PUBLISHED","Schedule"],["OPEN","Open now"],["CANCELLED","Cancel competition"]],PUBLISHED:[["OPEN","Open now"],["DRAFT","Return to draft"],["CANCELLED","Cancel competition"]],OPEN:[["CLOSED","Close entries"],["CANCELLED","Cancel competition"]],CLOSED:[["JUDGING","Move to judging"],["OPEN","Reopen entries with confirmation"],["CANCELLED","Cancel competition"]],JUDGING:[["CANCELLED","Cancel competition"]],COMPLETED:[],CANCELLED:[["DRAFT","Restore to draft with confirmation"]]} as Record<string,string[][]>)[competition.status].map(([to,label])=><PendingForm action={transitionCompetition} key={to} className="min-w-0" label={label} pendingLabel="Updating competition…" secondary={to === "CANCELLED"}><input type="hidden" name="competitionId" value={competition.id}/><input type="hidden" name="to" value={to}/>{to === "CANCELLED" && <label className="mb-2 grid gap-1 font-bold">Cancellation reason<input name="cancellationReason" required maxLength={500} className="rounded-xl p-3" placeholder="Required; records and images are retained"/></label>}</PendingForm>)}</div>{competition.cancellationReason&&<p className="mt-4 rounded-xl bg-lightgrey p-3"><strong>Cancelled {competition.cancelledAt?.toLocaleString("en-GB")}:</strong> {competition.cancellationReason}. Nothing was deleted.</p>}</Card>
      <CompetitionForm competition={competition} /><Card className="mt-5"><h2 className="text-xl font-bold text-navy">Hero image</h2><p className="mt-2 text-sm">Optional. The premium public layout always uses a controlled crop.</p>{competition.heroStorageKey&&<><img src={`/api/competition-hero/${competition.id}`} alt={competition.heroAltText||"Current competition hero"} className="mt-4 aspect-[16/9] w-full max-w-xl rounded-2xl object-cover" style={{objectPosition:competition.heroFocalPosition}}/><p role="status" className="mt-2 text-sm font-bold text-verified">Hero image ready</p></>}<HeroUploadForm action={uploadCompetitionHero} id={competition.id} hasHero={Boolean(competition.heroStorageKey)}/>{competition.heroStorageKey&&<form action={removeCompetitionHero} className="mt-2"><input type="hidden" name="competitionId" value={competition.id}/><button className="button-secondary">Remove image</button></form>}</Card>
      <Card className="mt-5"><h2 className="text-xl font-bold text-navy">Prize showcase</h2><p className="mt-2 text-sm">Manage each prize separately. The legacy summary remains available as a safe fallback.</p><form action={savePrize} className="mt-4 grid gap-3 md:grid-cols-2"><input type="hidden" name="competitionId" value={competition.id}/><input name="placement" required placeholder="First prize / runner-up" className="rounded-xl p-3"/><input name="title" placeholder="Prize title (optional)" className="rounded-xl p-3"/><textarea name="description" required placeholder="What the winner receives" className="rounded-xl p-3 md:col-span-2"/><input name="valuePounds" type="number" min="0" step="0.01" placeholder="Estimated value in pounds" className="rounded-xl p-3"/><input name="sponsor" placeholder="Sponsor / brand (optional)" className="rounded-xl p-3"/><input name="digitalAward" placeholder="Digital badge / award (optional)" className="rounded-xl p-3"/><input name="displayOrder" type="number" defaultValue="0" className="rounded-xl p-3"/><button className="button-primary md:col-span-2">Add prize</button></form>{competition.prizes.map(p=><div key={p.id} className="mt-3 flex items-center justify-between rounded-xl bg-lightgrey p-3"><div><strong>{p.placement}: {p.title||"Prize"}</strong><p className="text-sm">{p.description}</p></div><form action={removePrize}><input type="hidden" name="competitionId" value={competition.id}/><input type="hidden" name="prizeId" value={p.id}/><button className="font-bold text-red-700">Remove</button></form></div>)}</Card><Card className="mt-5"><h2 className="text-xl font-bold text-navy">Judging panel</h2><p className="mt-2 text-sm">Add judges in the order they should appear publicly.</p><form action={saveJudge} className="mt-4 grid gap-3"><input type="hidden" name="competitionId" value={competition.id}/><input name="name" required placeholder="Judge name" className="rounded-xl p-3"/><input name="roleTitle" placeholder="Role / title" className="rounded-xl p-3"/><input name="organisation" placeholder="Organisation / company" className="rounded-xl p-3"/><textarea name="biography" placeholder="Short biography" className="rounded-xl p-3"/><textarea name="profession" placeholder="What they do professionally" className="rounded-xl p-3"/><textarea name="judgingFocus" placeholder="What they will focus on" className="rounded-xl p-3"/><input type="number" name="displayOrder" defaultValue="0" className="rounded-xl p-3"/><label><input type="checkbox" name="guestJudge"/> Guest judge</label><button className="button-primary">Add judge</button></form>{competition.judges.map(j=><div key={j.id} className="mt-3 flex justify-between rounded-xl bg-lightgrey p-3"><div><span><strong>{j.name}</strong> · order {j.displayOrder}</span>{j.imageStorageKey&&<img src={`/api/judge-images/${j.id}`} alt={`${j.name} profile`} className="mt-2 h-20 w-20 rounded-full object-cover"/>}<form action={uploadJudgeImage} encType="multipart/form-data" className="mt-2 flex flex-wrap gap-2"><input type="hidden" name="competitionId" value={competition.id}/><input type="hidden" name="judgeId" value={j.id}/><input type="file" name="image" accept="image/jpeg,image/png,image/webp" required/><button className="font-bold text-info">{j.imageStorageKey?"Replace image":"Upload image"}</button></form>{j.imageStorageKey&&<form action={removeJudgeImage}><input type="hidden" name="competitionId" value={competition.id}/><input type="hidden" name="judgeId" value={j.id}/><button className="text-sm font-bold text-red-700">Remove image</button></form>}</div><form action={removeJudge}><input type="hidden" name="competitionId" value={competition.id}/><input type="hidden" name="judgeId" value={j.id}/><button className="font-bold text-red-700">Remove</button></form></div>)}</Card>
    </Section>
