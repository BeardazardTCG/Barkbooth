...
export function RegisterDogForm() {
  return <ManagedForm action={registerDog} pendingMessage="Creating…" encType="multipart/form-data" className="grid gap-5">
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <label className="font-bold text-navy">Sex<select name="sex" defaultValue="" className="mt-2 w-full rounded-2xl border border-navy/10 px-4 py-3"><option value="">Not provided</option>{sexOptions.map((sex) => <option key={sex} value={sex}>{sex}</option>)}</select></label>
      ...
      <label className="font-bold text-navy">Visibility of identity<select name="visibility" defaultValue="PUBLIC" className="mt-2 w-full rounded-2xl border border-navy/10 px-4 py-3"><option value="PUBLIC">Public registry identity</option><option value="PRIVATE">Private to your account</option><option value="LINK_ONLY">Link only</option></select></label>
