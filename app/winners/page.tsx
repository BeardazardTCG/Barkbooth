...
export default function WinnersPage() {
  return <>
    ...
    {galleryGroups.map(group => <Section key={group.tab} eyebrow={group.tab} title={`${group.tab} as profile records`}><div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">{group.items.map((w, i) => <Card key={`${group.tab}-${w.dog}-${i}`}><PawAvatar label={w.dog} /><Rosette label={w.place} className={w.ribbon} /><h3 className="mt-2 text-2xl font-bold text-navy">{w.dog}</h3><p className="font-bold text-rosette">{w.className}</p><p className="mt-2 text-charcoal/65">{w.note}</p><p className="mt-3 rounded-2xl bg-lightgrey p-3 text-sm font-bold text-navy">Saved as an optional profile achievement</p><Link href="/dog-profile" className="mt-4 inline-flex rounded-full bg-navy px-4 py-2 text-sm font-bold text-white">View dog profile</Link></Card>)}</div></Section>)}
  </>;
}
