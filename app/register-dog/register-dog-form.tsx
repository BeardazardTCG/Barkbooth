...
export function RegisterDogForm() {
  ...
  return <form action={action} className="grid gap-5">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <label className="font-black text-navy">Primary role<select name="primaryRole" required defaultValue="" className="mt-2 w-full rounded-2xl border border-cocoa/10 px-4 py-3"><option value="" disabled>Select role</option>{roles.map((role) => <option key={role}>{role}</option>)}</select></label>
      ...
      <label className="font-black text-navy">Visibility<select name="visibility" defaultValue="PUBLIC" className="mt-2 w-full rounded-2xl border border-cocoa/10 px-4 py-3"><option value="PUBLIC">Public</option><option value="PRIVATE">Private</option><option value="LINK_ONLY">Shared by link</option></select></label>
    </div>
