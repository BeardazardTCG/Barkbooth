...
function RecordFields({ record }: { record?: DogRecord }) {
  return <div className="grid gap-3 md:grid-cols-2">
    <label className="grid gap-1 text-sm font-bold text-navy">Category<select name="category" defaultValue={record?.category ?? "IDENTITY"} className="rounded-2xl border border-cocoa/10 bg-white p-3">{recordCategories.map((category) => <option key={category} value={category}>{recordCategoryLabels[category]}</option>)}</select></label>
    <label className="grid gap-1 text-sm font-bold text-navy">Record type<input name="recordType" required defaultValue={record?.recordType ?? ""} list="record-types" className="rounded-2xl border border-cocoa/10 bg-white p-3" /></label>
    <label className="grid gap-1 text-sm font-bold text-navy">Provider<input name="provider" defaultValue={record?.provider ?? ""} className="rounded-2xl border border-cocoa/10 bg-white p-3" /></label>
    <label className="grid gap-1 text-sm font-bold text-navy">Reference number<input name="referenceNumber" defaultValue={record?.referenceNumber ?? ""} className="rounded-2xl border border-cocoa/10 bg-white p-3" /></label>
    <label className="grid gap-1 text-sm font-bold text-navy">Owner statement<select name="status" defaultValue={record?.status ?? "HAVE_RECORD"} className="rounded-2xl border border-cocoa/10 bg-white p-3"><option value="HAVE_RECORD">I have this record</option><option value="DO_NOT_HAVE">I don&apos;t have this record</option></select></label>
    ...
    <label className="grid gap-1 text-sm font-bold text-navy">Issue date<input type="date" name="issueDate" defaultValue={dateValue(record?.issueDate ?? null)} className="rounded-2xl border border-cocoa/10 bg-white p-3" /></label>
    <label className="grid gap-1 text-sm font-bold text-navy">Expiry date<input type="date" name="expiryDate" defaultValue={dateValue(record?.expiryDate ?? null)} className="rounded-2xl border border-cocoa/10 bg-white p-3" /></label>
    <label className="grid gap-1 text-sm font-bold text-navy md:col-span-2">Notes<textarea name="notes" defaultValue={record?.notes ?? ""} className="min-h-20 rounded-2xl border border-cocoa/10 bg-white p-3" /></label>
