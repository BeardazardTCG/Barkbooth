...
const appLinks = [["Dashboard", "/dashboard", "home"], ["My Dogs", "/dashboard#my-dogs", "dogs"], ["Register Dog", "/register-dog", "plus"], ["Registry", "/profiles", "search"], ["Activities", "/competitions", "award"], ["Directory", "/directory", "directory"]];
const mobileLinks = [["Home", "/dashboard", "home"], ["My Dogs", "/dashboard#my-dogs", "dogs"], ["Register", "/register-dog", "plus"], ["Registry", "/profiles", "search"], ["Account", "/account", "person"]];
...
function PublicHeader({ viewer }: { viewer: Viewer }) {
  return <header className="sticky top-0 z-30 border-b border-navy/10 bg-cream/95 backdrop-blur"><nav aria-label="Primary navigation" className="mx-auto flex max-w-7xl items-center justify-between gap-5 px-5 py-3 lg:px-8"><Link href="/"><BarkBoothLogo /></Link><div className="hidden items-center gap-1 md:flex">{publicLinks.map(([label, href]) => <Link key={href} href={href} className="nav-link">{label}</Link>)}</div><div className="flex items-center gap-2">{viewer ? <Link href="/dashboard" className="button-secondary !px-4"><Initials viewer={viewer} /><span className="hidden sm:inline">Dashboard</span></Link> : <><Link href="/login" className="nav-link hidden sm:inline-flex">Log in</Link><Link href="/signup" className="button-primary">Create account</Link></>}</div></nav><div className="flex gap-1 overflow-x-auto border-t border-navy/5 px-4 py-2 md:hidden">{publicLinks.slice(0, 3).map(([label, href]) => <Link key={href} href={href} className="nav-link flex-1 justify-center">{label}</Link>)}</div></header>;
}

function AppSidebar({ viewer, pathname }: { viewer: NonNullable<Viewer>; pathname: string }) {
  return <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col bg-navy px-4 py-5 text-white lg:flex"><Link href="/dashboard" className="px-2"><BarkBoothLogo inverse /></Link><nav aria-label="Owner workspace" className="mt-9 grid gap-1">{appLinks.map(([label, href, icon]) => { const base = href.split("#")[0]; const active = pathname === base; return <Link key={href} href={href} className={`app-nav-link ${active ? "bg-white/[0.12] text-white" : "text-skysoft/75 hover:bg-white/[0.08] hover:text-white"}`}><Icon name={icon}/>{label}</Link>; })}</nav><div className="mt-auto border-t border-white/10 pt-4"><Link href="/account" className={`app-nav-link ${pathname.startsWith("/account") ? "bg-white/[0.12]" : ""}`}><Initials viewer={viewer}/><span className="min-w-0"><span className="block truncate text-sm font-bold">{viewer.displayName}</span><span className="block truncate text-xs text-skysoft/60">Account & settings</span></span></Link><form action={logout}><button className="app-nav-link mt-1 w-full text-skysoft/70"><Icon name="logout"/>Log out</button></form></div></aside>;
}

function MobileAppNav({ pathname }: { pathname: string }) { return <nav aria-label="Mobile owner navigation" className="mobile-app-nav lg:hidden">{mobileLinks.map(([label, href, icon]) => { const active = pathname === href.split("#")[0]; return <Link key={href} href={href} aria-current={active ? "page" : undefined} className={`mobile-app-link ${active ? "text-navy" : "text-slate"}`}><Icon name={icon}/><span>{label}</span></Link>; })}</nav>; }
...
export function SiteChrome({ viewer, children }: { viewer: Viewer; children: ReactNode }) {
  ...
  const isApp = !!viewer && (pathname === "/dashboard" || pathname === "/register-dog" || pathname.startsWith("/account"));
  if (isApp) return <><AppSidebar viewer={viewer} pathname={pathname}/><div className="min-h-screen lg:pl-64"><header className="flex h-16 items-center justify-between border-b border-navy/10 bg-white px-5 lg:hidden"><Link href="/dashboard"><BarkBoothLogo /></Link><Link href="/account" aria-label="Open account settings"><Initials viewer={viewer}/></Link></header><main className="app-main">{children}</main><footer className="app-footer">Bark Booth Canine Registry · <Link href="/legal/privacy-policy">Privacy</Link> · <Link href="/legal/terms-and-conditions">Terms</Link></footer></div><MobileAppNav pathname={pathname}/></>;
