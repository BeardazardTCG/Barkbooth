...
const bbRegistryPattern = /^BB-\d{6}$/;
export async function saveHeritageParent(_previous: ActionResult, formData: FormData) { return runPetAction(async()=>{const dog=await requireDogOwner(asString(formData.get("dogId")));const parentType=asEnum(asString(formData.get("parentType")),["MOTHER","FATHER"] as const,"MOTHER");const barkBoothRegistryNumber=asString(formData.get("barkBoothRegistryNumber"));if(barkBoothRegistryNumber&&!bbRegistryPattern.test(barkBoothRegistryNumber))throw new Error("Bark Booth registry number must use the BB-000000 format.");await prisma.dogHeritageParent.upsert({where:{dogId_parentType:{dogId:dog.id,parentType}},create:{dogId:dog.id,parentType,isUnknown:formData.get("isUnknown")==="on",parentName:asString(formData.get("parentName"))||null,barkBoothRegistryNumber:barkBoothRegistryNumber||null,breed:asString(formData.get("breed"))||null,colourMarkings:asString(formData.get("colourMarkings"))||null,registryAffiliated:asEnum(asString(formData.get("registryAffiliated")),["YES","NO","UNKNOWN"] as const,"UNKNOWN"),registryName:asString(formData.get("registryName"))||null,registryNumber:asString(formData.get("registryNumber"))||null,notes:asString(formData.get("notes"))||null},update:{isUnknown:formData.get("isUnknown")==="on",parentName:asString(formData.get("parentName"))||null,barkBoothRegistryNumber:barkBoothRegistryNumber||null,breed:asString(formData.get("breed"))||null,colourMarkings:asString(formData.get("colourMarkings"))||null,registryAffiliated:asEnum(asString(formData.get("registryAffiliated")),["YES","NO","UNKNOWN"] as const,"UNKNOWN"),registryName:asString(formData.get("registryName"))||null,registryNumber:asString(formData.get("registryNumber"))||null,notes:asString(formData.get("notes"))||null}});revalidateDogSurfaces(dog.registryNumber)},"Heritage details saved");}
export async function removeHeritageParent(_previous: ActionResult, formData: FormData){return runPetAction(async()=>{const row=await prisma.dogHeritageParent.findUnique({where:{id:asString(formData.get("heritageId"))},include:{dog:true}});if(!row)throw new Error("Heritage detail not found.");await requireDogOwner(row.dogId);await prisma.dogHeritageParent.delete({where:{id:row.id}});revalidateDogSurfaces(row.dog.registryNumber)},"Heritage details removed");}
export async function saveVaccination(_previous: ActionResult, formData: FormData){return runPetAction(async()=>{const dog=await requireDogOwner(asString(formData.get("dogId")));const name=asString(formData.get("name"));if(!name)throw new Error("Vaccination name is required.");const id=asString(formData.get("vaccinationId"));const data={name,vaccinated:asString(formData.get("vaccinated"))==="YES",administeredAt:asDate(asString(formData.get("administeredAt"))),nextDueAt:asDate(asString(formData.get("nextDueAt"))),veterinaryPractice:asString(formData.get("veterinaryPractice"))||null,notes:asString(formData.get("notes"))||null};if(id){const old=await prisma.dogVaccination.findUnique({where:{id}});if(!old||old.dogId!==dog.id)throw new Error("Vaccination not found.");await prisma.dogVaccination.update({where:{id},data})}else await prisma.dogVaccination.create({data:{dogId:dog.id,...data}});revalidateDogSurfaces(dog.registryNumber)},"Vaccination saved",true)}
export async function removeVaccination(_previous: ActionResult, formData: FormData){return runPetAction(async()=>{const row=await prisma.dogVaccination.findUnique({where:{id:asString(formData.get("vaccinationId"))},include:{dog:true}});if(!row)throw new Error("Vaccination not found.");await requireDogOwner(row.dogId);await prisma.dogVaccination.delete({where:{id:row.id}});revalidateDogSurfaces(row.dog.registryNumber)},"Vaccination removed")}
