...
export default function CalendarPupsPage() {
  return <>
    ...
    <Section eyebrow="Choose a month" title="Twelve seasonal profile-memory cards"><div className="grid gap-4 sm:grid-cols-2 md:grid-cols-4">{calendarMonths.map(month => <Card key={month}><div className="grid h-24 place-items-center rounded-[1.5rem] bg-gradient-to-br from-skysoft to-biscuit text-4xl">📅</div><h3 className="mt-4 text-2xl font-bold text-navy">{month}</h3><p className="mt-2 text-sm text-charcoal/65">Optional calendar activity for a profile memory.</p><button type="button" className="mt-3 rounded-full bg-white px-4 py-2 text-sm font-bold text-navy shadow-soft">Select month mock</button></Card>)}</div></Section>
