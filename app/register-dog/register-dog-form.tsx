...
function Submit() {
  ...
  return <button disabled={pending} className="rounded-full bg-info px-5 py-3 text-sm font-bold text-white shadow-soft disabled:opacity-60">{pending ? "Registering..." : "Create Bark Booth Identity"}</button>;
}
...
export function RegisterDogForm() {
  ...
  return <form action={action} encType="multipart/form-data" className="grid gap-5">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <label className="font-bold text-navy">Profile photo <span className="text-sm text-charcoal/50">optional</span><input type="file" name="photo" accept="image/jpeg,image/png,image/webp" className="mt-2 block w-full text-sm text-charcoal" /><span className="mt-2 block text-sm font-normal text-charcoal/55">JPEG, PNG or WebP. Maximum 5 MB.</span></label>
    </div>
