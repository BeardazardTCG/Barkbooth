  ...
  return <>
      ...
      <Card className="bg-gradient-to-br from-white via-cream to-skysoft/50">
        <div className="grid gap-5 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            ...
            <label className="mt-5 block text-sm font-bold text-navy">Dog name or Bark Booth number<input placeholder="Try Mabel or BB-000001" className="mt-2 w-full rounded-2xl border border-cocoa/10 bg-white px-4 py-3 font-bold text-charcoal" /></label>
    ...
    <Section eyebrow="Public identity cards" title="Every card starts with identity, status, and latest record">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{profileCards.map(dog => <Card key={dog.profileNumber} className={`flex flex-col ${dog.status === "Rainbow Bridge" ? "bg-gradient-to-br from-white to-skysoft/40" : ""}`}>
        ...
        <div className="mt-4 flex-1">
          <div className="flex flex-wrap gap-2"><span className="rounded-full bg-lightgrey px-3 py-2 text-xs font-bold text-navy">{dog.profileNumber}</span><span className="rounded-full bg-biscuit px-3 py-2 text-xs font-bold text-navy">{dog.status}</span>{dog.rescueBadge && <span className="rounded-full bg-emerald-100 px-3 py-2 text-xs font-bold text-navy">🐾 {dog.rescueBadge}</span>}{dog.memorial && <span className="rounded-full bg-skysoft px-3 py-2 text-xs font-bold text-navy">🌈 Memorial</span>}</div>
          <h3 className="mt-3 text-3xl font-bold text-navy">{dog.name}</h3>
          <p className="mt-1 font-bold text-rosette">{dog.breed}</p>{dog.kennelClubName && <p className="mt-1 text-sm font-bold text-charcoal/60">KC name: {dog.kennelClubName}</p>}<div className="mt-2 flex flex-wrap gap-2">{dog.dogTypes?.map((type) => <span key={type} className="rounded-full bg-white px-3 py-1 text-xs font-bold text-navy">{type}</span>)}</div>
        ...
        {dog.adoptionCta ? <button type="button" className="mt-5 rounded-full bg-emerald-600 px-5 py-3 text-center text-sm font-bold text-white shadow-soft">{dog.adoptionCta}</button> : <Link href="/dog-profile" className="mt-5 rounded-full bg-navy px-5 py-3 text-center text-sm font-bold text-white shadow-soft">View example identity</Link>}
      </Card>)}</div>
    </Section>
    ...
    <Section eyebrow="Memorial example" title="Rainbow Bridge profiles stay calm and respectful">
      <Card className="bg-gradient-to-br from-white to-skysoft/50">
        <div className="grid gap-5 md:grid-cols-[180px_1fr]"><PawAvatar label="Daisy memorial photos" /><div><p className="text-sm font-bold uppercase tracking-[0.2em] text-rosette">In Loving Memory</p><h3 className="mt-2 text-4xl font-bold text-navy">Daisy · BB-000042</h3><p className="mt-2 font-bold text-charcoal/65">3 March 2012 – 18 November 2025</p><p className="mt-4 leading-8 text-charcoal/70">A peaceful memorial passport with selected achievements, selected photos, and one family message — never a social feed.</p><div className="mt-4 flex flex-wrap gap-2">{memorialDog?.memorial?.achievements.map(achievement => <span key={achievement} className="rounded-full bg-white px-3 py-2 text-sm font-bold text-navy">{achievement}</span>)}</div><div className="mt-4 grid gap-2 sm:grid-cols-3">{memorialDog?.memorial?.photos.map(photo => <span key={photo} className="rounded-2xl bg-white/70 p-3 text-sm font-bold text-navy">📷 {photo}</span>)}</div></div></div>
      </Card>
    </Section>
  </>;
}
