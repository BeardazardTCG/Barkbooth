...
export default function TricksAndTailsPage() {
  return <>
    ...
    <Section eyebrow="Judging" title="Welfare-first video criteria"><div className="grid gap-4 md:grid-cols-4">{tricksCriteria.map(item => <Card key={item.label}><p className="text-3xl font-bold text-info">{item.value}</p><h3 className="mt-2 font-bold text-navy">{item.label}</h3></Card>)}</div></Section>
  </>;
}
