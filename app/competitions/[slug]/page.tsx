...
export default async function CompetitionPage({ params }: { params: { slug: string } }) {
  ...
  return <>
    ...
    {competition.results.length > 0 && <Section eyebrow="Published results" title="Results">{competition.results.map((result) => <Card key={result.id}><p className="font-bold text-info">{result.placement}. {result.title}</p><p className="text-xl font-bold text-navy">{result.entry.dog.name}</p>{result.judgeNotes && <p>{result.judgeNotes}</p>}</Card>)}</Section>}
  </>;
}
